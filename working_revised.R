...
company_list_num <- c(1:100)
com <- c()
for (j in company_list_num){
  # com <- c(com, paste0(List_500_companies$symbol[j], collapse=","))
com[j] <- paste0(List_500_companies$symbol[j], collapse=",")


# com <- paste0(com, collapse = ",")
## Get the stock prices 
data_ss[[j]] <- rjson::fromJSON(file = 
                             paste0("https://fmpcloud.io/api/v3/historical-price-full/",com[j],"?from=2021-04-01&to=",
